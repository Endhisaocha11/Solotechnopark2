      ...
      <header className="mt-20 pt-16 bg-contact-patern">
        <div className="container lg:h-[85vh] pb-20 grid lg:grid-cols-12 gap-5 lg:gap-10 items-center">
          <div className="mx-auto flex flex-col justify-center items-start lg:col-span-5 text-white">
            ...
            { state.isLang === 'ID' ? 'Informasi ' : 'Information'}<br/>{ state.isLang === 'ID' ? 'Kontak' : 'Contact'} 
            </h1>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20">
              <div>
                <Card className={"shadow-lg p-5 md:p-10 border"}>
                  <div className="header border-b ">
                    ...
                    { state.isLang === 'ID' ? 'Tidak bisa menemukan' : 'Could not find'}<br />
                      <strong className="text-primary-100">{ state.isLang === 'ID' ? 'Informasi' : 'Information'} </strong>
                      { state.isLang === 'ID' ? 'yang Anda cari?' : 'what you are looking for?'}
                    </h2>
                  </div>
                  {status === 200 ? (
                    ...
                    <form>
                      <div className="my-5">
                        <label className="mb-2 block head-4"> { state.isLang === 'ID' ? 'Nama' : 'Name'}</label>
                        <input
                          required
                          className="bg-slate-100 w-full outline-primary-100 block text-md py-3 px-4 rounded-full"
                          type="text"
                          placeholder={ state.isLang === 'ID' ? 'Masukkan nama lengkap di sini' : 'Enter full name here'}
                      ...
                      <div className="my-5">
                        <label className="mb-2 block head-4">{ state.isLang === 'ID' ? 'Email' : 'Email'}</label>
                        <input
                          required
                          className="bg-slate-100 w-full outline-primary-100 block text-md py-3 px-4 rounded-full"
                          type="email"
                          placeholder={ state.isLang === 'ID' ? 'Masukkan email di sini' : 'Enter email here'}
                      ...
                      <div className="my-5">
                        <label className="mb-2 block head-4">{ state.isLang === 'ID' ? 'Judul' : 'Title'}</label>
                        <input
                          className="bg-slate-100 w-full outline-primary-100 block text-md py-3 px-4 rounded-full"
                          type="text"
                          placeholder={ state.isLang === 'ID' ? 'Tentang apa yang ingin Anda tanyakan' : 'About what you want to ask'}
                      ...
                      <div className="my-5">
                        <label className="mb-2 block head-4">{ state.isLang === 'ID' ? 'Pesan' : 'Message'}</label>
                        <textarea
                          className="bg-slate-100 w-full h-60 outline-primary-100 block text-md py-3 px-4 rounded-lg"
                          placeholder={ state.isLang === 'ID' ? 'Masukkan pesan yang ingin Anda sampaikan di sini' : 'Enter the message you want to convey here'}
              ...
              <div className="md:my-16">
                <div className="header border-b ">
                  ...
                  <h2 className="capitalize head-2 my-5">
                  { state.isLang === 'ID' ? 'Butuh' : 'Need'} &nbsp;
                    <strong className="text-primary-100">{ state.isLang === 'ID' ? 'bantuan' : 'help'}</strong> ?
                    { state.isLang === 'ID' ? 'kunjungi kami' : 'visit us'}  
                  </h2>
                </div>
                <ul className="my-5">
